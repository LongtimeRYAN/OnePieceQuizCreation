...
def initialize_questions(game):
    ...
    game.add_question(
        "What object does Genzo have on his hat?", "A pinwheel", ["arlong park"]
    )
    game.add_question(
        "What was Nami caught stealing from her village as a child?",
        "Books. Charting/navigation books",
        ["arlong park"],
    )
    game.add_question(
        "How did the Arlong pirates spot Bellemere’s house?",
        "Smoke from cooking",
        ["arlong park"],
    )
    game.add_question(
        "Why did the Arlong Pirates want Nami?", "The maps she drew", ["arlong park"]
    )
    game.add_question(
        "Who sent the marines to find Nami’s money?", "Arlong", ["arlong park"]
    )
    game.add_question(
        "Who prevented the villagers from rushing into Arlong park just to die?",
        "Johnny and Yosaku",
        ["arlong park"],
    )
    game.add_question(
        "How does Luffy get his feet stuck in the ground while fighting Arlong?",
        "He stomped the ground too hard",
        ["arlong park"],
    )
    game.add_question(
        "How does Arlong take Luffy out of the fight almost immediately?",
        "Picking him up and throwing him into the sea",
        ["arlong park"],
    )
    game.add_question("How many swords does Hachi use?", "6", ["arlong park"])
    game.add_question(
        "How do Genzo and Nojiko get Luffy air while he is trapped?",
        "They stretch his neck",
        ["arlong park"],
    )
    game.add_question(
        "How did Sanji stun Kuroobi long enough to prevent himself from drowning?",
        "Forced air into his gills",
        ["arlong park"],
    )

    # Add questions and answers for the Syrup Village arc
    game.add_question(
        "How does the butler Klahadore adjust his glasses?",
        "By using his palm to push them up",
        ["syrup village"],
    )
    game.add_question(
        "How did Luffy know Usopp's dad back at Syrup Village?",
        "Yassop is a member of the Red Hair Pirates",
        ["syrup village"],
    )
    game.add_question(
        "Why was Kuro undercover as a butler for three years?",
        "To steal Kaya’s wealth without arousing any suspicion",
        ["syrup village"],
    )
    game.add_question(
        "For what occasion did Kaya buy a present for Klahadore?",
        "His 3-year anniversary of working for her",
        ["syrup village"],
    )
    game.add_question(
        "Who was the first ally to engage the Kuro Pirates after they changed locations?",
        "Usopp",
        ["syrup village"],
    )
    game.add_question(
        "How did Zoro get out of the oil slick in Syrup Village?",
        "He used his swords to climb out",
        ["syrup village"],
    )
    game.add_question(
        "How did the Kuro Pirates suddenly get stronger during the fight?",
        "Jango's hypnotism",
        ["syrup village"],
    )
    game.add_question(
        "Name one of the Nyaban Brothers.", "Buchi and Sham", ["syrup village"]
    )
    game.add_question(
        "What do the Symbols on Kuro’s coat represent?", "It’s poop", ["syrup village"]
    )
    game.add_question(
        "Why does Kuro adjust his glasses the way he does?",
        "A habit he picked up from having swords for hands",
        ["syrup village"],
    )
...
def display_arc_selection():
    ...
    if st.button("Start Quiz"):
        ...
        if selected_arc == "All Arcs":
            ...
            for question, answer, keywords in zip(
                st.session_state.game.questions,
                st.session_state.game.answers,
                st.session_state.game.keywords,
            ):
